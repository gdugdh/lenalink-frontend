  ...
  return (
    <div className="rounded-lg border bg-white p-3 sm:p-4 md:p-6">
      ...
      <div className="space-y-3 sm:space-y-4">
              ...
              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 sm:gap-0">
                <div className="min-w-0">
                  ...
                  <div className="text-xs sm:text-sm text-[#022444] truncate">
                    {displayRoute.departureCity} {routeCodes[0] ? `(${routeCodes[0]})` : ''}
                  </div>
                </div>
                <div className="text-center w-full sm:w-auto order-2 sm:order-2">
                  ...
                  <div className="text-[10px] sm:text-xs font-medium text-[#022444]">
                    Самолет {displayRoute.carrier ? `• ${displayRoute.carrier}` : ''}
                  </div>
                </div>
                <div className="min-w-0 text-left sm:text-right order-3 sm:order-3">
                  ...
                  <div className="text-xs sm:text-sm text-[#022444] truncate">
                    {displayRoute.arrivalCity} {routeCodes[routeCodes.length - 1] ? `(${routeCodes[routeCodes.length - 1]})` : ''}
                  </div>
                </div>
              </div>
            </div>
        ...
        {/* Bus segment - only show for multi-segment routes like Moscow -> Olekminisk */}
        {displayRoute.departureCity === 'Москва' && displayRoute.arrivalCity === 'Олекминск' && routeCodes.length > 2 && (
          <div className="flex items-start gap-2 sm:gap-3 md:gap-4 border-b pb-3 sm:pb-4">
        ...
        {/* River segment - only show for Moscow -> Olekminisk route */}
        {displayRoute.departureCity === 'Москва' && displayRoute.arrivalCity === 'Олекминск' && routeCodes.length > 2 && (
